...
def main(permutation, split, alpha, layers):
    ...
    #obtaining NSP outputs (process values, means and covariance matrices) # XXX XXX XXX XXX XXX 
    ns_partial_outputs = compute_samples_ns_prior(ns_prior, n_layers_ns, [x,z], n_samples, 
                                          dim_data,prior_ns_structure, dropout_rate_) 
    ...
    with tf.Session(config = config) as sess:
        ...
        #     feed_dict={x: X_test, y_: y_test, n_samples: 100, dropout_rate_ : dropout_rate_test})
        ...
        for epoch in range(n_epochs):
            ...
            for i_batch in range(n_batches_train):
                ...
                if epoch % print_epochs == 0:
                     ...
                     errors_tmp, LL_tmp = sess.run([ squared_error, test_ll_estimate] , \
                         feed_dict={x: batch[0], y_: (batch[1]*stdyTrain + meanyTrain), n_samples: n_samples_test, dropout_rate_: dropout_rate_train})
            ...
            if epoch % print_epochs == 0:
                 ...
                 tmp_test_se, tmp_test_ll = sess.run([ squared_error , test_ll_estimate ] , \
                                      feed_dict={x: X_test, y_: y_test, n_samples: n_samples_test, dropout_rate_: dropout_rate_test})
                 ...
                 with open("prints/print_IP_" + str(alpha) + "_" + str(split) + "_" + "dropout_" + str(dropout_rate_train) + "_" +  original_file, "a") as res_file:
                     res_file.write('alpha %g datetime %s epoch %d ELBO %g Loss %g KL %g real_time %g \
                     cpu_train_time %g annealing_factor %g C.E. %g \n train_RMSE %g; train_LL %g \n test_RMSE %g test_LL %g' % (alpha, \
                     str(datetime.now()), epoch, L, loss, kl, (fini_ref - ini_ref), (fini - ini), \
                     kl_factor, ce, RMSE, TestLL, test_se, test_ll) + "\n")
        ...
        input, ips, resx_noise, resx, resz, labels, results = sess.run([ x, z, y_prior_fx, f_prior_fx, \
            y_prior_fz, y_, y_test_estimated ], feed_dict={x: X_test, y_: y_test, n_samples: 100, dropout_rate_: dropout_rate_test})
        ...
        res_mean, res_std = sess.run([res_mean_tmp, res_std_tmp], feed_dict={x: X_test, y_: y_test, n_samples: n_samples_test, dropout_rate_: dropout_rate_train})
