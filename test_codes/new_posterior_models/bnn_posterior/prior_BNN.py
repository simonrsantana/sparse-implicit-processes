...
def prior_bias_initializer(shape):
  initial = tf.random.normal(shape = shape, mean = 0.0, stddev = 0.5, dtype = tf.float64) # tf.zeros(shape = shape) # To initialize all the biases to 0 identically 
...
def compute_samples_bnn(prior_network, n_layers, data, n_samples, dim_data, prior_structure, number_ips):
    ...
    noise_3 = tf.random_normal(shape = [ n_samples, n_units2, 1 ], dtype = tf.float64) #, seed = seed)
