...
def w_variable_mean(shape):
  initial = tf.random.normal(shape = shape, mean = 0.0, stddev = 0.3, dtype = tf.float64) # mean 0 stddev 1
  return tf.Variable(initial)

def w_variable_variance(shape):
  initial = tf.random.normal(shape = shape, mean = 0.0, stddev = 1.5, dtype = tf.float64 ) - 5.0   # mean 0 stddev 1
  return tf.Variable(initial)
...
from prior_BNN import *				# Prior BNN model functions
# from posterior_neural_sampler import *        # Neural sampler that draws instances from q()
from posterior_bnn import *			# BNN model for the posterior
...
# Structural parameters of the prior BNN
bnn_structure = [50, 50, 1]                     # Structure of the BNN
n_layers_bnn = len(bnn_structure)               # Number of layers in the NN

# Structure of the posterior BNN
posterior_structure = [50, 50, number_IP]  # Structure of the neural sampler
n_layers_posterior = len(posterior_structure)     # Number of layers in the NS
...
def main(permutation, split, alpha, layers):
    ...
    data = np.loadtxt(original_file).astype(np.float64)
    ...
    data_test = np.loadtxt( 'test_' + original_file ).astype(np.float64)
    ...
    x = tf.placeholder(tf.float64, [ None, dim_data ])
    y_ = tf.placeholder(tf.float64, [ None, 1 ])
    n_samples = tf.placeholder(tf.int32, [ 1 ])[ 0 ]
    kl_factor_ = tf.placeholder(tf.float64, [ 1 ])[ 0 ]
    ...
    n_layers_bnn = n_layers_posterior = n_layers_disc_prior = n_layers_disc_approx = layers
    ...
    posterior_model = create_posterior(dim_data, posterior_structure, n_layers_posterior)
    ...
    # XXX # Introduce the BNN as posterior model as well
    z_tmp = tf.stop_gradient(z)
    samples_qu = compute_samples_posterior(posterior_model, n_layers_posterior, z_tmp, n_samples, dim_data, posterior_structure)    # right now, dims are (number_IP, n_samples)
    ...
    K_zz_add_noise = K_zz + tf.eye( tf.shape(K_zz)[0] ) * 1e-5 # XXX
    ...
    inner_cholesky = cov_est + tf.eye( tf.shape(x)[0] ) * 1e-5 # XXX
    ...
    K_zz_add_noise = K_zz + tf.eye( tf.shape(K_zz)[0] , dtype = tf.float64) * 1e-5 # XXX
    ...
    #### WE ARE DOING THE MEAN THROUGH THE SAMPLES OF f(z) (= u)
    # Instead of using u from the prior p(), we use u from the approximate distribution q() for the differences in the expression
    mean_est = tf.expand_dims(m_fx, -1) + tf.tensordot(tf.cast(cov_product, tf.float64),  (samples_qu - tf.expand_dims(m_fz, -1)), axes = [[1], [0]])   # Dimensions: first term: (batchsize, 1); sec. term: (batchsize, n_samples)
    cov_est = tf.cast(K_xx, tf.float64) - tf.matmul( cov_product, tf.cast(K_xz, tf.float64), transpose_b = True )

    # import pdb; pdb.set_trace()

    sample_pf_noise = tf.random_normal(shape = [ tf.shape(x)[0], n_samples ], dtype = tf.float64 )
    ...
    log_sigma2_noise = tf.Variable(tf.cast(1.0 / 100.0, dtype = tf.float64))

    # Final loss for the data term
    loss_train = (1.0/alpha) * ( -tf.log(tf.cast(n_samples, tf.float64 )) + tf.reduce_logsumexp( -0.5 * alpha * (np.log( 2 * np.pi ) + log_sigma2_noise  + (samples_pf - y_)**2 / tf.exp(log_sigma2_noise) ), axis = [ 1 ]))
    sum_loss = tf.reduce_sum( loss_train )

    # Compute the test metrics
    pre_noise = tf.random_normal(shape = [ tf.shape(x)[0], n_samples ], dtype = tf.float64) * tf.sqrt(tf.exp(log_sigma2_noise))
    ...
    # L.L.
    raw_test_ll = tf.reduce_logsumexp( -0.5*(tf.log(2 * np.pi * tf.exp(log_sigma2_noise) * stdyTrain**2) + (y_ - y_test_estimated)**2 / (tf.exp(log_sigma2_noise) * stdyTrain**2)), axis = [ 1 ]) - tf.log(tf.cast(n_samples, tf.float64))
    ...
    mean_p = tf.stop_gradient(mean_p); var_p = tf.stop_gradient(var_p);
    mean_q = tf.stop_gradient(mean_q); var_q = tf.stop_gradient(var_q);
    ...
    samples_p_gaussian = tf.random_normal(shape = tf.shape(norm_p), mean = 0, stddev = 1, dtype = tf.float64)  #, seed = seed)
    samples_q_gaussian = tf.random_normal(shape = tf.shape(norm_q), mean = 0, stddev = 1, dtype = tf.float64)  #, seed = seed)
    ...
    ELBO =  sum_loss - kl_factor_ * mean_KL * tf.cast(tf.shape(x)[ 0 ], tf.float64) / tf.cast(size_train, tf.float64)

    neg_ELBO = -ELBO
    mean_ELBO = ELBO

    vars_primal = get_variables_posterior(posterior_model) + get_variables_bnn(bnn) + [ log_sigma2_noise, z ]
    ...
    with tf.Session(config = config) as sess:
        ...
        merge_fz = pd.concat([pd.DataFrame(ips), pd.DataFrame(labels), pd.DataFrame(resz)], axis = 1)
        merge_fz.to_csv('res_IP/' + str(alpha) + '/' + str(alpha) + "_initial_prior_samples_fz.csv", index = False)
