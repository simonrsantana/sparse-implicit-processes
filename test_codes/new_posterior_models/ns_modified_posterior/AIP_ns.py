...
def w_variable_mean(shape):
  initial = tf.random.normal(shape = shape, mean = 0.0, stddev = 0.15, dtype = tf.float64) # mean 0 stddev 1
  return tf.Variable(initial)

def w_variable_variance(shape):
  initial = tf.random.normal(shape = shape, mean = 0.0, stddev = 2, dtype = tf.float64) - 5   # mean 0 stddev 1
  return tf.Variable(initial)
...
# from BNN_prior import *                           # Import  the functions from the BNN prior model
...
# from posterior_structure import *            # neural sampler for the prior
from alternative_posterior_neural_sampler import *
...
posterior_structure = [50, 50, m]  # Structure of the neural sampler
n_layers_ns = len(posterior_structure)     # Number of layers in the NS
...
def main(permutation, split, alpha, layers):
    ...
    # neural_sampler = create_neural_sampler(posterior_structure, noise_comps_ns, n_layers_ns)   # Import the implicit process posterior distriution model
    neural_sampler = create_posterior(dim_data, posterior_structure, n_layers_ns)   # Import the implicit process posterior distriution model
    ...
    # XXX # Check whether the calculations done below are alright
    samples_qu = compute_samples_posterior(neural_sampler, n_layers_ns, z_tmp, n_samples, dim_data, posterior_structure)    # right now, dims are (number_IP, n_samples)
    ...
    inv_chol_Kzz =  tf.linalg.triangular_solve( chol_Kzz, rhs = tf.eye(tf.shape(chol_Kzz)[0], dtype = tf.float64) , lower = True )# , tf.float32)
    ...
    inner_cholesky = cov_est + tf.eye( tf.shape(x)[0], dtype = tf.float64 ) * 1e-5 # XXX
