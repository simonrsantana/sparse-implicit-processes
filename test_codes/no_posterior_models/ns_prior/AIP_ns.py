...
def main(permutation, split, alpha, layers):
    ...
    # XXX # SRS: Force the use of the prior
    ...
    K_zz_add_noise = K_zz + tf.eye( tf.shape(K_zz)[0], dtype = tf.float64) * 1e-5 
    ...
    inv_chol_Kzz =  tf.linalg.triangular_solve( chol_Kzz, rhs = tf.cast( tf.eye(tf.shape(chol_Kzz)[0]), tf.float64 ) , lower = True )# , tf.float64)
