...
def calculate_covariances(delta_1, delta_2):
    ...
    #if tf.cast(tf.shape(delta_1)[ 1 ], tf.float64) != tf.cast(tf.shape(delta_2)[ 1 ], tf.float64):
    ...
    K_ff = (1 / (tf.cast(tf.shape(delta_1)[ 1 ], tf.float64) - 1 )) * tf.matmul( delta_1,  delta_2, transpose_b = True)
