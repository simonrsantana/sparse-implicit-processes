...
def exp_log_likelihood_double(mean_targets, std_targets, weights_train, weights_test, alpha):
    ...
    res_train = ( 1.0/alpha) * (- np.log( samples_train ) + tf.reduce_logsumexp( alpha * (-0.5 * (np.log(2.0 * np.pi) + log_sigma2_noise + \
        (A3 - tf.reshape(y_, shape = [ tf.shape(x)[0], 1, 1 ]))**2 / tf.exp(log_sigma2_noise))) , axis = [ 1 ]))
...
#                kl = sess.run(mean_KL, feed_dict={x: batch[0], y_: batch[1]})
            ...
            with open("prints/res_alpha_reduced_disc_print-doubled_mod/results_alpha_" + str(alpha) + "_" + str(split) + "_" +  original_file, "a") as res_file:
        ...
        np.savetxt('res_alpha_reduced_disc-doubled_mod/' + str(alpha) + 'results_rmse_' + str(split) + '.txt', [ RMSE ])
        np.savetxt('res_alpha_reduced_disc-doubled_mod/' + str(alpha) + 'results_ll_' + str(split) + '.txt', [ TestLL ])
        np.savetxt('res_alpha_reduced_disc-doubled_mod/' + str(alpha) + 'results_time_' + str(split) + '.txt', [ timing ])
        np.savetxt('res_alpha_reduced_disc-doubled_mod/' + str(alpha) + 'results_KL_' + str(split) + '.txt', [ tmp_kl ])
        np.savetxt('res_alpha_reduced_disc-doubled_mod/' + str(alpha) + 'results_ELBO_' + str(split) + '.txt', [ tmp_elbo ])
...
if __name__ == '__main__':
    ...
    if not os.path.isdir("res_alpha_reduced_disc-doubled_mod"):
        os.makedirs("res_alpha_reduced_disc-doubled_mod")

    # Create a folder to store the screen prints
    if not os.path.isdir("prints"):
        os.makedirs("prints")

    if not os.path.isdir("prints/res_alpha_reduced_disc_print-doubled_mod"):
        os.makedirs("prints/res_alpha_reduced_disc_print-doubled_mod")

    # Create (or empty) the results of the run
    if os.path.isfile("prints/res_alpha_reduced_disc_print-doubled_mod/results_alpha_" + str(alpha) + "_" + str(split) + "_" +  original_file):
        with open("prints/res_alpha_reduced_disc_print-doubled_mod/results_alpha_" + str(alpha) + "_" + str(split) + "_" +  original_file, "w") as res_file:
           res_file.close()
