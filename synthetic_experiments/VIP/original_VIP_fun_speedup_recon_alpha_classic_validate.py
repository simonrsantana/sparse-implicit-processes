...
def VIP_pred(ft_train,Y,ft_test,m_ft_train,m_ft_test,tau,yita):
    ...
    global gftrain, gjitter, gsig, gtau
    gsig = sig
    gtau = tau
    gftrain = ft_train
    gjitter = tf.eye(c)*(sig**2+1/(yita**2))
...
with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess: # True
    ...
    # Prior samples
    prior_samples = sess.run(ft_test, feed_dict={A:A_samp, X_test:x_test,Y_test:y_test, X_train:x_train, Y_train:y_train,Z_w[0]:Z_w_samp[0], Z_w[1]:Z_w_samp[1],Z_w[2]:Z_w_samp[2], Z_b[0]:Z_b_samp[0], Z_b[1]:Z_b_samp[1],Z_b[2]:Z_b_samp[2]})
