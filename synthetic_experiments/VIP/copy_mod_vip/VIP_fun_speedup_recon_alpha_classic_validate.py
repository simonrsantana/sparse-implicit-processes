...
def bnn(X,Z_w,Z_b,structure,n_bt,n_train):
    ...
    var_y = tf.reshape(tf.nn.moments(y,1)[1],[n_train,1]) # SRS
    y = y - m_y
    y = y/np.sqrt(n_bt-1)
    return y, m_y, var_y      # SRS El output aquí es los phi del paper de la eq. 13
...
def VIP_pred(ft_train,Y,ft_test,m_ft_train,m_ft_test,tau,yita):
    ...
    # SRS changein the following line
...
with tf.variable_scope("VIP") as scope:
    ...
    ## training
    ft_train,m_ft_train, var_ft_train = bnn(X_train,Z_w,Z_b,structure,n_bt,n_train)
    ...
    ft_va,m_ft_va, var_ft_va = bnn(X_test,Z_w,Z_b,structure,n_bt,n_train)
    ...
    ## test
    ft_test,m_ft_test, var_ft_test = bnn(X_test,Z_w,Z_b,structure,n_bt,n_train)
