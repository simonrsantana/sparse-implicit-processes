...
def w_variable_mean(shape):
  initial = tf.random.normal(shape = shape, mean = 0.0, stddev = 0.2, dtype = data_type) # mean 0 stddev 1
  return tf.Variable(initial, dtype = data_type)

def w_variable_variance(shape):
  initial = tf.random.normal(shape = shape, mean = 0.0, stddev = 0.1, dtype = data_type) - 5   # mean 0 stddev 1
  return tf.Variable(initial, dtype = data_type)
