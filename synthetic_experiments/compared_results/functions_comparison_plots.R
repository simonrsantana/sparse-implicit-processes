...
# dd = read.table("/home/simon/Desktop/implicit-variational-inference/FBNN/FBNN/test_bim_data.txt")
# plot(dd, pch = 20)
...
for (j in 1:length( ip_folders )){
  ...
  for (i in c(1:length(alphas))){
    ...
    tmp_plot <- ggplot(mdata_fx_final, aes(x=x, y=value, color=variable)) +
      geom_line( alpha = 0.15) + theme_classic() + 
      xlim(-4,4) + ylim(-15, 15) +
      theme(legend.position = "none") + scale_color_viridis(option="C", discrete = T)
...
vip_prior_plot <- ggplot(melt_vip_prior, aes(x=x, y=value, color=variable)) +
  geom_line( alpha = 0.3) + theme_classic() +
  theme(legend.position = "none") + scale_color_viridis(option="C", discrete = T) +
...
fbnn_prior_plot <- ggplot(melt_fbnn_prior, aes(x=x, y=value, color=variable)) +
  geom_line( alpha = 0.15) + theme_classic() +
  theme(legend.position = "none") + scale_color_viridis(option="C", discrete = T) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), limits = c(-15, 15)) 
...
for (j in 1:length( ip_folders )){
  ...
  for (i in c(1:length(alphas))){
    ...
    tmp_plot <- ggplot(mdata_fx_final, aes(x=x, y=value, color=variable)) +
      ...
      scale_x_continuous(expand = c(0, 0)) +
      scale_y_continuous(expand = c(0, 0), limits = c(-15, 15)) +
      theme(legend.position = "none") + scale_color_viridis(option="A", discrete = T)
...
pred_vip <- read.csv("vip_y_forecasted_0.txt")
names_vip <- paste0("sample_", c(1:(ncol(pred_vip) - 3)), "_yx")

names(pred_vip) <- c("x","y", names_vip, "uc")
...
# vip_pred_plot <- ggplot(melt_vip_pred, aes(x=x, y=value, color=variable)) +
#   geom_line( alpha = 0.15) + theme_classic() +
#   theme(legend.position = "none") + scale_color_viridis(option="C", discrete = T) +
#   scale_x_continuous(expand = c(0, 0)) +
#   scale_y_continuous(expand = c(0, 0)) # , limits = c(-15, 15)) 
...
vip_pred_plot <- ggplot(pred_vip, aes(x=x, y=sample_1_yx, ymin=(pred_vip$sample_1_yx - pred_vip$uc), ymax=(pred_vip$sample_1_yx + pred_vip$uc), fill="red")) + # color=variable)) +
  geom_line( alpha = 1.0) + theme_classic() +
...
pred_fbnn <- read.csv("fbnn_y_forecasted_0.txt")[,1:100]
...
fbnn_pred_plot <- ggplot(melt_fbnn_pred, aes(x=x, y=value, color=variable)) +
  geom_line( alpha = 0.3) + theme_classic() +
  theme(legend.position = "none") + scale_color_viridis(option="C", discrete = T) +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), limits = c(-15, 15)) 
...
ggsave(filename = "comparison_plot_alternative_fbnn.png", plot = gridplot,
